...
typedef struct remix_sem
{
    ...
    U32 uiSemOpt;
    U8* pucSemMem;
    struct remix_tcb* pstrSemTask;  //»¥³âÐÅºÅÁ¿
}REMIX_SEM;
...
{
    REMIX_TASKSCHEDTAB strFlagTab;      //µÈ´ýµÄÈÎÎñ
    U32 uiFlagOpt;                      //FIFO OR PRIO
    U32 uiFlagNowBit;                 //µ±Ç°ÄÄÐ©×´Ì¬±êÖ¾Î»¾ÍÐ÷
    U8* pucFlagMem;                     //·ÖÅäµÄÄÚ´æ¿Õ¼ä
}REMIX_FLAG;

typedef struct remix_taskflag
{
    struct remix_flag* pRemixFlag;      //Ö¸ÏòÐèÒªµÄÐÅºÅÁ¿¼¯
    U32 uiFlagWantBit;                //ÐèÒªÄÄÐ©×´Ì¬±êÖ¾Î»
    U32 uiFlagNodeOpt;                     //ÐèÒªÕâÐ©×´Ì¬±êÖ¾Î»µÄÀàÐÍÊÇAND/OR?
}REMIX_TASKFLAG;

#endif

//ÓÐÒ»Ð©¿ÉÒÔ¿¼ÂÇºÏ²¢£¬ÖØÐÂ·â×°
typedef struct remix_tcb
{
    ...
    REMIX_TCBQUE strSemQue;         //register in Semaphore OR Flag queue
    REMIX_SEM* pstrSem;
#ifdef REMIX_SEMGROUPFLAG
    REMIX_TASKFLAG strTaskFlag;
#endif
    U8* pucTaskName;
    U8* pucTaskStack;
    U32 uiTaskFlag;                 //ÊÇ·ñÔÚdelay±íÖÐ?ÊÇ·ñ¼Ì³ÐÓÅÏÈ¼¶?ÊÇ·ñÉêÇëÕ»å
    PRIORITYBITS ucTaskPrio;
#ifdef REMIX_TASKPRIOINHER
    PRIORITYBITS ucTaskPrioBackup;
#endif
    REMIX_TASKOPT strTaskOpt;           //ÀïÃæµÄucTaskSta¼ÇÂ¼5ÖÐ×´Ì¬:ready?delay?pend...
    U32 uiStillTick;
#ifdef REMIX_CPUSTATISTIC
