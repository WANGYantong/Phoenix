...
void REMIX_CpuShareStatistic(REMIX_TCB* pstrOldTcb, REMIX_TCB* pstrNewTcb)
{
    ...
    if(NULL != pstrOldTcb)
    {
        if(suiTick == guiTick)
        {
            pstrOldTcb->strCpuShare.uiCounter +=
                              (pstrOldTcb->strCpuShare.uiSysTickVal - uiSysTickVal);
        }
        else
        {
            pstrOldTcb->strCpuShare.uiCounter +=
    ...
    if(CPUSHARETIME == guiCpuSharePeriod)
    {
        ...
        while(NULL !=
              (pstrTaskList = REMIX_DlistNextNodeEmpInq(&gstrTaskList, pstrTaskList)))
        {
            pstrTaskTcb = ((REMIX_TCBQUE*)pstrTaskList)->pstrTcb;

            pstrTaskTcb->strCpuShare.uiCpuShare =
                            (pstrTaskTcb->strCpuShare.uiCounter + SYSTICKPERIOD / 2)
