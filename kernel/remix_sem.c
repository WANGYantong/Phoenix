...
REMIX_SEM* REMIX_SemCreate(REMIX_SEM* pstrSem, U32 uiSemOpt, U32 uiInitVal)
{
    ...
    if(((SEMBIN!=(SEMTYPEMASK & uiSemOpt))&&(SEMCNT!=(SEMTYPEMASK & uiSemOpt))&&(SEMMUT!=(SEMTYPEMASK & uiSemOpt)))
        ...
        || (((SEMMUT!=(SEMTYPEMASK & uiSemOpt))||(SEMPRIO!=(SEMSCHEDULEMASK & uiSemOpt)))&&(SEMPRIINH==(SEMPRIORITYINHMASK & uiSemOpt)))){
            ...
            if(SEMFULL==(SEMTYPEMASK & pstrSem->uiSemOpt)){
                ...
                if(gpstrCurTcb==pstrSem->pucSemMem){
                    ...
                    if(SEMPRIINH==(SEMPRIORITYINHMASK & pstrSem->uiSemOpt)){
                        REMIX_TaskPrioInheritance(pstrSem->pstrSemTask, gpstrCurTcb->ucTaskPrio);
                    }
